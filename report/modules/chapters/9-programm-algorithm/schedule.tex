\subsection{Модуль schedule}

\subsubsection{Get}

\begin{lstlisting}
    Delivery *Get(int index) const;
\end{lstlisting}
...
\begin{itemize}
    \item \verb|auto *new_node = new Delivery(delivery);|
...
\begin{itemize}
    ...
    \item Если \verb|index == 0|
        \begin{itemize}
            \item \verb|Delivery *new_head = head->next;|
    ...
    \item Если \verb|index == size - 1|
        \begin{itemize}
            \item \verb|Delivery *new_tail = tail->prev;|
...
\begin{itemize}
    \item \verb|Delivery *p = head;|
    \item Пока \verb|p != nullptr|
        \begin{itemize}
            \item \verb|Delivery *next = p->next;|
            \item \verb|delete p;|
            \item \verb|p = next;|
        \end{itemize}
